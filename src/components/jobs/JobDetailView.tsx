...
export const JobDetailView = ({
  ...
  const [isEditing, setIsEditing] = useState(!job.title || job.title === "Untitled Job" || !job.requirements);
  ...
  return <div className="container mx-auto p-6 space-y-6 max-w-4xl">
      ...
      <Card className={isEditing ? "border-primary/50 shadow-lg" : ""}>
        <CardHeader className={isEditing ? "p-8" : "p-6"}>
          {isEditing ? <div className="space-y-6">
              <Input placeholder="Job Title" value={editTitle} onChange={e => setEditTitle(e.target.value)} className="text-2xl font-semibold h-14 px-4" />
              <Textarea placeholder="Job Description / Requirements" value={editDescription} onChange={e => setEditDescription(e.target.value)} rows={12} className="text-base min-h-[300px] p-4" />
      ...
      <Card>
        ...
        <CardContent>
          <ResumeUpload resumes={job.resumes} onUploadResumes={onUploadResumes} onDeleteResume={onDeleteResume} onGetApplicationLink={handleOpenShareModal} />
        </CardContent>
      </Card>

      {/* Action Buttons */}
      {/* Share Link Modal */}
      <ShareLinkModal open={shareLinkModalOpen} onOpenChange={setShareLinkModalOpen} jobId={job.id} jobTitle={job.title} slug={job.slug} />
      ...
      <Button onClick={onGoToDashboard} size="lg" className="w-full h-16 text-lg font-semibold bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 shadow-lg">
